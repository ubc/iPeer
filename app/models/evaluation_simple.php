...
class EvaluationSimple extends AppModel
{
  ...
  /*var $belongsTo = array(
    ...
    return $this->findAll('grp_event_id='.$grpEventId.' AND evaluator='.$evaluator);
	}

	// gets simple_evaluation_mark objects for a specific assignment and evaluator
	function getResultsByEvaluatee($grpEventId=null, $evaluatee=null) {
    return $this->findAll('grp_event_id='.$grpEventId.' AND evaluatee='.$evaluatee);
	}
	...
	function getAllComments ($grpEventId=null, $evaluatee=null) {
	  return $this->findAll('grp_event_id='.$grpEventId.' AND evaluatee='.$evaluatee, 'eval_comment');
	}
