...
class V1ControllerTest extends CakeTestCase {
    ...
    private function _get_http_response_code($url) {
        $url .= (strpos($url, '?') === false ? '?' : '&') . 'oauth=0';
        $headers = get_headers($url);
        return substr($headers[0], 9, 3);
    }

    private function _apiReq($url, $content = null, $reqType = null)
    {
        // Bypass OAuth using debug mode parameter
        $url .= (strpos($url, '?') === false ? '?' : '&') . 'oauth=0';
    ...
    public function testUsers()
    {
        ...
        $file = $this->_apiReq($url, json_encode($newUser), 'POST');
        ...
        $fileSame = $this->_apiReq($url, json_encode($newUserSame), 'POST');
        ...
        $fileAgain = $this->_apiReq($url, json_encode($newUserAgain), 'POST');
        ...
        $fileSameL = $this->_apiReq($url, json_encode($newUserSameL), 'POST');
        ...
        $fileAgainL = $this->_apiReq($url, json_encode($newUserAgainL), 'POST');
        ...
        $fileBefore = $this->_apiReq($url, json_encode($newUserBefore), 'POST');
        ...
        $fileAfter = $this->_apiReq($url, json_encode($newUserAfter), 'POST');
        ...
        $file = $this->_apiReq($url, json_encode($newUsers), 'POST');
        ...
        $fileSame = $this->_apiReq($url, json_encode($newUsersSame), 'POST');
        ...
        $fileAgain = $this->_apiReq($url, json_encode($newUsersAgain), 'POST');
        ...
        $fileSameL = $this->_apiReq($url, json_encode($newUsersSameL), 'POST');
        ...
        $fileAgainL = $this->_apiReq($url, json_encode($newUsersAgainL), 'POST');
        ...
        $fileBefore = $this->_apiReq($url, json_encode($newUsersBefore), 'POST');
        ...
        $fileAfter = $this->_apiReq($url, json_encode($newUsersAfter), 'POST');
        ...
        $file = $this->_apiReq($url, $users, 'POST');
        $users = json_decode($file, true);
        ...
        $file = $this->_apiReq($url, json_encode($updatedPerson), 'PUT');
        ...
        $fileSame = $this->_apiReq($url, json_encode($updatedUserSame), 'PUT');
        ...
        $fileSameL = $this->_apiReq($url, json_encode($updatedUserSameL), 'PUT');
        ...
        $fileUp = $this->_apiReq($url, json_encode($updatedUserUp), 'PUT');
        $this->assertEqual(json_decode($fileUp, true), $expectedUserUp);

        // DELETE - delete the user
        $ret = $this->_apiReq("$url/$userId", null, 'DELETE');
        ...
        $this->_apiReq("$url/$importUserId", null, 'DELETE');
        $this->assertEqual($this->_get_http_response_code("$url/$userId"), '404');
    }
    ...
    public function testCourses()
    {
        ...
        $actualCourse = $this->_apiReq("$url/1");
        ...
        $file = $this->_apiReq($url, json_encode($newCourse), 'POST');
        ...
        $file = $this->_apiReq("$url/$id", json_encode($updateCourse), 'PUT');
        $checkCourse = json_decode($file, true);
        $id = $checkCourse['id'];
        ...
        $file = $this->_apiReq("$url/$id", null, 'DELETE');

        $this->assertEqual($this->_get_http_response_code("$url/$id"), '404');
    ...
    public function testGroups() {
        ...
        $file = $this->_apiReq("$url/2/groups", json_encode($addGroup), 'POST');
        ...
        $file = $this->_apiReq("$url/2/groups/$id", json_encode($editGroup), 'PUT');
        ...
        $ret = $this->_apiReq("$url/2/groups/$id", null, 'DELETE');
        $this->assertEqual(json_decode($ret), array());
        $this->assertEqual($this->_get_http_response_code("$url/2/groups/$id"), '404');
    }
    ...
    public function testGroupMembers()
    {
        ...
        $addedMembers = $this->_apiReq("$url", json_encode($toBeAdded), 'POST');
        $expected = array(
            array('username' => 'redshirt0001'),
            array('username' => 'redshirt0002'),
            array('username' => 'redshirt0003'),
        ...
        $update = array(
            ...
            array('username' => 'redshirt0011'),
            array('username' => 'redshirt0013'));
        $updated = $this->_apiReq("$url", json_encode($update), 'POST');
        $updated = $this->_apiReq("$url");
        $expected = array(
            array('id' => '5', 'role_id' => '5', 'username' => 'redshirt0001', 'last_name' => 'Student', 'first_name' => 'Ed'),
            array('id' => '6', 'role_id' => '5', 'username' => 'redshirt0002', 'last_name' => 'Student', 'first_name' => 'Alex'),
            array('id' => '7', 'role_id' => '5', 'username' => 'redshirt0003', 'last_name' => 'Student', 'first_name' => 'Matt'),
        ...
        $ret = $this->_apiReq("$url/redshirt0011",null,'DELETE');
        $this->assertEqual(json_decode($ret, true), array());
        $ret = $this->_apiReq("$url/redshirt0013",null,'DELETE');
        $this->assertEqual(json_decode($ret, true), array());
    ...
    public function testEvents()
    {
        ...
        $actualEvents = $this->_apiReq("$url/1/events");
        ...
        $actualEvent = $this->_apiReq("$url/1/events/3");
        $expectedEvent = array("id" => "3", "title" => "Project Evaluation", "course_id" => "1",
            "event_template_type_id" => "4", "due_date" => $this->nextYear."-07-02 09:00:28");
    ...
    public function testGrades()
    {
        ...
        // evaluation simple
        $studentGrade = $this->_apiReq("$url/1/grades/redshirt0029");
        ...
        // evaluation rubric
        $studentGrade = $this->_apiReq("$url/2/grades/redshirt0001");
        $studentGrade = json_decode($studentGrade, true);
        $expectedGrade = array("evaluatee" => "5", "score" => "14",
            'username' => 'redshirt0001');
        $this->assertEqual($expectedGrade, $studentGrade);
        // evaluation mixeval
        $studentGrade = $this->_apiReq("$url/3/grades/redshirt0002");
        $studentGrade = json_decode($studentGrade, true);
    ...
    function testUsersEvents()
    {
        $expectedEvents = array(
            array(
                ...
                'due_date' => $this->nextYear.'-07-02 16:34:43',
                'release_date_begin' => "2011-06-16 16:34:49",
                'release_date_end' => $this->nextYear."-07-22 16:34:53",
                'result_release_date_begin' => $this->nextYear.'-07-04 16:34:43',
                'result_release_date_end' => $this->nextYear.'-07-30 16:34:43',
                'is_released' => true,
            ...
            array(
                ...
                'due_date' => $this->nextYear.'-06-08 08:59:29',
                'release_date_begin' => "2011-06-06 08:59:35",
                'release_date_end' => $this->nextYear."-07-02 08:59:41",
                'result_release_date_begin' => $this->nextYear.'-06-09 08:59:29',
                'result_release_date_end' => $this->nextYear.'-07-08 08:59:29',
                'is_released' => true,
                'is_result_released' => false,
                'is_ended' => false,
                'id' => '2',
            ...
            array(
                ...
                'due_date' => $this->nextYear.'-07-02 09:00:28',
                'release_date_begin' => "2011-06-07 09:00:35",
                'release_date_end' => $this->nextYear."-07-09 09:00:39",
                'result_release_date_begin' => $this->nextYear.'-07-04 09:00:28',
                'result_release_date_end' => $this->nextYear.'-07-12 09:00:28',
                'is_released' => true,
                'is_result_released' => false,
                'is_ended' => false,
                'id' => '3',
            ...
            array(
                ...
                'due_date' => $this->nextYear.'-07-31 11:20:00',
                'release_date_begin' => "2012-07-01 11:20:00",
                'release_date_end' => $this->nextYear."-12-31 11:20:00",
            ...
            array(
                ...
                'release_date_end' => $this->nextYear.'-11-29 00:00:00',
                'result_release_date_begin' => $this->nextYear.'-11-30 00:00:00',
                'result_release_date_end' => $this->nextYear.'-12-12 00:00:00',
                'is_released' => true,
                'is_result_released' => false,
                'is_ended' => false,
                'id' => '6',
            ...
            array(
                ...
                'release_date_begin' => $this->nextYear.'-07-31 11:20:00',
        ...
        $courseUserEvents = $this->_apiReq("$url");
        ...
        $eventsSubmittable = $this->_apiReq("$url");
        ...
        $filteredEvents = $this->_apiReq("$url");
    ...
    function testEnrolments() {
        ...
        $expected = array(array('username' => 'redshirt0005', 'role_id' => 5));
        $actual = $this->_apiReq(
            $url, json_encode($expected), 'DELETE');
        $this->assertEqual($expected, json_decode($actual, true));
