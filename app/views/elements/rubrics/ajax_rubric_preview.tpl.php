    ...
    <table width="95%" border="0" align="center" cellpadding="4" cellspacing="2">
		...
		for($i=1; $i<=$LOM_num; $i++){
			...
			$html->areaTag('Rubric/lom_comment'.$i,'',2,
                array('style'=>'width:90%;','value'=>(!empty($data['Rubric']['lom_comment'.$i]) ? $data['Rubric']['lom_comment'.$i] : $LOM_des[$i])))."</td>";
		}
		...
		if( $rubric_type == "horizontal" ){
			...
			for($i=1; $i<=$criteria_num; $i++){
				...
				echo '<td class="tableheader2" valign="top"><table border="0" width="95%" cellpadding="2"><tr><td align="left">Criteria '.$i.'</td></tr><tr><td>'
					  .$html->areaTag('Rubric/criteria'.$i,15,2, array('value'=>(isset($data['Rubric']['criteria'.$i]) ? $data['Rubric']['criteria'.$i] : $crit[$i]),'style'=>'width:95%;'))."<br>"
					  ."</td></tr></table></td>";
