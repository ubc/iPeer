...
<table width="95%" border="0" align="center" cellpadding="4" cellspacing="2">
	...
	<tr class="tablecell">
      ...
      <td width="5">Max.</td>
		...
		<td align="center"><input type="text" name="points[]" id="point<?=$user['id']?>" value="<?php echo empty($params['data']['Evaluation']['point'.$user['id']])? '' : $params['data']['Evaluation']['point'.$user['id']] ?>" size="5" onchange="updateCount(<?=$remaining?>,<?=$event['Event']['com_req']?>);">
    </td>
		<td><input type="text" name="comments[]" id="comment<?=$user['id']?>" value="<?php echo empty($params['data']['Evaluation']['comment_'.$user['id']])? '' : $params['data']['Evaluation']['comment_'.$user['id']] ?>" size="50" onchange="updateCount(<?=$remaining?>,<?=$event['Event']['com_req']?>);">
      <script type="text/javascript" language="javascript">
