...
class EvaluationsController extends AppController
{
    ...
    public $components = array('ExportBaseNew', 'Auth', 'AjaxList', 'Output',
        ...
        'RequestHandler', 'EvaluationsRequest', 'JsonResponse');
    public $body = [];
    public $method = '';
    ...
    function makeEvaluation($eventId, $objectId = null, $studentId = null)
    {
        ...
        if ($this->RequestHandler->accepts('json')) {
            ...
            switch ($templateTypeId) {
                case 1:
                    $this->EvaluationsRequest->processSimpleEvaluationSubmitRequest($this->method, $event, $objectId, $studentId);
                    break;
                case 2:
                    $this->EvaluationsRequest->processRubricEvaluationSubmitRequest($this->method, $event, $objectId, $studentId);
                    break;
                case 4:
                    $this->EvaluationsRequest->processMixedEvaluationSubmitRequest($this->method, $event, $objectId, $studentId);
                    break;
            }
    ...
    function studentViewEvaluationResult($eventId, $groupId = null, $studentId = null)
    {
        ...
        if (!is_numeric($eventId) ||
            ...
            if ($this->RequestHandler->accepts('json')) {
                $this->JsonResponse->withMessage('Invalid id or you do not have permission to access this event.')->withStatus(400);
                return;
            } elseif ($this->RequestHandler->accepts('html')) {
                $this->Session->setFlash(__('Error: Invalid id or you do not have permission to access this event.', true));
                $this->redirect('/home/index');
        ...
        if ('3' != $event['Event']['event_template_type_id']) {
            ...
            if (!is_numeric($groupId) ||
                ...
                !($group = $this->Group->getGroupWithMemberRoleByGroupIdEventId($groupId, $eventId))) {
                if ($this->RequestHandler->accepts('json')) {
                    $this->JsonResponse->withMessage('Invalid group id or you are not in this group.')->withStatus(400);
                    return;
                } elseif ($this->RequestHandler->accepts('html')) {
            ...
            if (!$event['Event']['is_result_released']) {
                if ($this->RequestHandler->accepts('json')) {
                    $this->JsonResponse->withMessage('The results are not released.')->withStatus(404);
                    return;
                } elseif ($this->RequestHandler->accepts('html')) {
        ...
        if ($this->RequestHandler->accepts('json')) {
            ...
            switch ($event['Event']['event_template_type_id']) {
                case 1:
                    $this->EvaluationsRequest->processSimpleEvaluationResultRequest($event, $groupEventId, $autoRelease, $userId, $this->method);
                    break;
                case 2:
                    $this->EvaluationsRequest->processRubricEvaluationResultRequest($event, $groupEventId, $autoRelease, $userId, $this->method);
                    break;
                case 4:
                    $this->EvaluationsRequest->processMixedEvaluationResultRequest($event, $groupEventId, $autoRelease, $userId, $this->method);
                    break;
            }
        } elseif ($this->RequestHandler->accepts('html')) {
