  ...
  <tr>
    ...
    <td>
      <div class="status flex items-center space-x-1 text-sm">
        <component :is="row?.event?.is_submitted === '1' && row?.event?.is_result_released ? IconCheckmark : IconTimes" class="w-5 h-5" />
        <template v-if="row?.event?.is_submitted === '1' && row?.event?.is_result_released">
          <router-link :class="'completed'" :to="{ name: 'evaluation.edit', params: { event_id: row?.event?.id, group_id: row?.group?.id } }">
            {{ 'Completed' }}
          </router-link>
        </template>
        <template v-else>
          <span :class="'not-done text-gray-500'">{{ 'Not Done' }}</span>
        </template>
