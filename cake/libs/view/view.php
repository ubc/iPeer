<?php
/* SVN FILE: $Id: view.php,v 1.5 2006/06/20 18:46:38 zoeshum Exp $ */
...
/**
 * File extension. Defaults to Cake's conventional ".tpl.php".
 *
 * @var array
 */
	var $ext = '.tpl.php';
 ...
 * for a view filename (e.g. customFunkyView.tpl.php).
 *
	...
	function render($action = null, $layout = null, $file = null) {
		...
		if (!is_file($viewFileName) && !fileExistsInPath($viewFileName) || $viewFileName === '/' || $viewFileName === '\\') {
			...
			foreach(array($this->name, 'errors') as $viewDir) {
				...
				} elseif($missingViewFileName = fileExistsInPath(LIBS . 'view' . DS . 'templates' . DS . $viewDir . DS . $errorAction . '.tpl.php')) {
				} else {
		...
		if ($viewFileName && !$this->hasRendered) {
			if (substr($viewFileName, -5) === 'tpl.php') {
 ...
 */
	function renderLayout($content_for_layout) {
		...
		if (DEBUG > 2 && $this->controller != null) {
			$debug = View::_render(LIBS . 'view' . DS . 'templates' . DS . 'elements' . DS . 'dump.tpl.php', array('controller' => $this->controller), false);
		} else {
			$debug = '';
		}
 ...
 */
	function error($code, $name, $message) {
		...
		print ($this->_render(VIEWS . 'layouts/error.tpl.php', array('code'    => $code,
																						'name'    => $name,
																						'message' => $message)));
	}
...
/**
 * Returns filename of given action's template file (.tpl.php) as a string. CamelCased action names will be under_scored! This means that you can have LongActionNames that refer to long_action_names.tpl.php views.
 *
 ...
 */
	function _getViewFileName($action) {
		...
		if ($viewFileName = fileExistsInPath(LIBS . 'view' . DS . 'templates' . DS . 'errors' . DS . $type . $action . '.tpl.php')) {
		} elseif($viewFileName = fileExistsInPath(LIBS . 'view' . DS . 'templates' . DS . $this->viewPath . DS . $type . $action . '.tpl.php')) {
		} else {
 ...
 *
 * @return string Filename for layout file (.tpl.php).
 * @access private
 */
	function _getLayoutFileName() {
		...
		} elseif($layoutFileName = fileExistsInPath(LIBS . 'view' . DS . 'templates' . DS . "layouts" . DS . $type . "{$this->layout}.tpl.php")) {
		} else {
